    ...
    def getCrossLabels(
        ...
        with open(self.file_path + file_name, 'r') as f:
            ...
            for line in lines:
                ...
                user_dict[key] = spots_array      #user_dict: {user_id: [poi, poi, poi, ..., poi], user_id: [poi, poi, poi, ..., poi], ...}
                for spot in spots_array:
                    if spot not in spot_dict:
                        spot_dict[spot] = []
                    spot_dict[spot].append(key)   #spot_dict: {poi_id: [user, user,...], poi_id: [user, user,...],...}
    ...
    def getUserGraph(
        ...
        with open(self.file_path + file_name, 'r') as f:
            ...
            for line in lines:
                key = int(line.split(' ')[0])
                if key in user_enum:
                    relation_dict[user_enum[key]] = [user_enum[i] for i in ast.literal_eval(line[len(str(key)) + 1:]) if
                                                     i in user_enum]
